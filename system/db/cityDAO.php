...
class cityDAO
{
    ...
    public function salvar($city){
        ...
        try {
            ...
            if ($city->getIdCity() != "") {
                $statement = $pdo->prepare("UPDATE tb_city SET str_cod_siafi_city = :csc, str_name_city = :nameCity, tb_state_id_state = :state WHERE id_city = :id;");
                $statement->bindValue(":id", $city->getIdCity());
            } else {
                $statement = $pdo->prepare("INSERT INTO tb_city ( str_name_city, str_cod_siafi_city, tb_state_id_state) VALUES (:nameCity, :csc, :state)");
            }
            $statement->bindValue(":csc",$city->getCodSiafiCity());
            $statement->bindValue(":state",$city->getCityState());
            $statement->bindValue(":nameCity",$city->getNameCity());
    ...
    public function atualizar($city)
    {
        ...
        try {
            ...
            if ($statement->execute()) {
                ...
                $city->setCityState($rs->tb_state_id_state);
                return $city;
            } else {
                throw new PDOException("Erro: Não foi possível executar a declaração sql");
            }
    ...
    public function tabelaPaginada()
    {
        ...
        $sql = "SELECT id_city, str_cod_siafi_city, str_name_city, tb_state_id_state FROM tb_city LIMIT {$linha_inicial}, " . QTDE_REGISTROS;
