    ...
    public function salvar($action){
        ...
        try {
            ...
            if ($action->getIdAction() != "") {
                $statement = $pdo->prepare("UPDATE tb_action SET str_cod_action = :code, str_name_action = :nameAction WHERE id_action = :id;");
                $statement->bindValue(":id", $action->getIdAction());
            } else {
                $statement = $pdo->prepare("INSERT INTO tb_action (str_cod_action, str_name_action) VALUES (:code, :nameAction)");
            }
            $statement->bindValue(":code",$action->getCode());
            $statement->bindValue(":nameAction",$action->getName());
    ...
    public function atualizar($action)
    {
        ...
        try {
            ...
            if ($statement->execute()) {
                ...
                $action->setCode($rs->str_cod_action);
                $action->setName($rs->str_name_action);
                return $action;
            } else {
                throw new PDOException("Erro: Não foi possível executar a declaração sql");
            }
